...
function ExecuteQueryMixTransform (options) {
  ...
  //output CSV header
  console.log('"T","connections scanned","connections MST","journey time","query execution time","Bytes transferred","number of requests","bytes per connection","ms per connection","%CPU"');
}
  ...
  if (typeof object.T === "undefined" || !object.departureTime || object.departureTime == 'null') {
    ...
    var printResult = function (result) {
      ...
      usage.lookup(pid, function(error, r) {
        console.log(result.T + ',' + result.connectionsProcessed + ',' + result.connectionsMST + ',' + result.journeyTime + ',' + result.queryDuration + ',' + result.httpTotalBytes + ',' + result.httpRequests + ',' +result.httpTotalBytes/result.connectionsProcessed + ',' + result.queryDuration/result.connectionsProcessed + ',' + r.cpu);
      });
    };
